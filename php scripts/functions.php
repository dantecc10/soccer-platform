            ...
            switch ($j) {
                case 0:
                    ...
                    $dom .= (' <td class="align-middle submain-bg-color" style="background-color: var(--third-color) !important;">
                                <div class="row">
                                    <div class="col col-12 col-md-3 py-1 px-0" style="max-height: 50px !important;"><a href="detail.php?team=' . $data[$i][(10)] . '" style="/*max-height: inherit;*/"><img class="bs-icon-sm icon rounded-4" src="' . $data[$i][$j] . '" style="max-height: 40px;width: auto;"></a></div>
...
function fetch_player_cards($id)
{
    ...
    $cards_dom = "";
    $sql = ('SELECT 
    `p`.`id_player`, `p`.`name_player`, `p`.`nickname_player`, `p`.`last_names_player`, `p`.`dorsal_player`, `p`.`img_player`,
    t.icon_team AS team_icon, `p`.`player_team_id` FROM players p LEFT JOIN teams t ON p.player_team_id = t.id_team WHERE (`player_team_id` = 3);');
