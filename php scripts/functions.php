...
function detailed_matches_output($time, $match_fetching_id)
{
    ...
    $sql = ($time != null && $time != "") ? ('SELECT m.start_schedule_match, m.field_match, t1.name_team AS local_name_team, t1.icon_team AS local_icon_team, m.local_goals_match,
    ...
    for ($i = 0; $i < sizeof($matches); $i++) {
        ...
        if (isset($match_fetching_id)) {
            if (($match_fetching_id != null) && ($match_fetching_id != '')) {
                $events = proccess_events(match_events(intval($match_fetching_id)), [intval($matches[$i][12]), intval($matches[$i][13])]);
                echo ($events);
                return $events;
            }
        }
